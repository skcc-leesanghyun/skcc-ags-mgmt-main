...
public class UserActvyLogServiceImpl implements UserActvyLogService {
    ...
    public void regUserActvyLog(UserActvyLogDto userActvyLogDto) {
        try {            
            ...
            // 시퀀스 값을 가져와서 DTO에 설정
            Long nextSeq = userActvyLogRepository.getNextSequenceValue();
            userActvyLog.setUserActvySeq(nextSeq);
            ...
            log.info("userActvyLog: {}", userActvyLog);
            // 엔티티를 저장
            userActvyLog = userActvyLogRepository.save(userActvyLog);            
        } catch (Exception e) {
            log.error("Failed to register user activity log: {}", e.getMessage(), e);
            throw new ServiceException("COM.I1030", e);
        }
    }
